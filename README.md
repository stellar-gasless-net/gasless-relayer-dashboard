...
- **Verified Credential Check (Session Keys tab, added 2026-09-12)** — real, live `has_credential` read against `stellar-zklab/stellar-zkident`'s deployed `credential_verifier`, the same gate `reputation_nft::mint()`, `sybil_resistant_faucet::claim()`, and `stellar-gasless-relayer`'s own sponsorship tiering all depend on. Reimplements `@stellar-gasless/sdk`'s `hasVerifiedCredential` (that package is genuinely published to npm now, but this dashboard has no bundler, and importing it via an ESM CDN would risk duplicating `@stellar/stellar-sdk`'s internals — see `real.js`'s own header comment) — pre-filled with the real verified demo subject so it shows a genuine "Verified" result on load, and honestly reports "Not verified" for any address with no matching credential record rather than erroring.
